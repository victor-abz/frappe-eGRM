...
def submit_grievance(**kwargs):
	...
	try:
		...
		if not frappe.db.exists(
			"GRM Project Link", {"parent": category, "parenttype": "GRM Issue Category", "project": project}
		):
			return {"status": "error", "message": _("Selected category does not belong to this project")}

		if not frappe.db.exists(
			"GRM Project Link", {"parent": issue_type, "parenttype": "GRM Issue Type", "project": project}
		):
		...
		# --- Find initial status ---
		initial_status = None
		initial_statuses = frappe.get_all("GRM Issue Status", filters={"initial_status": 1}, fields=["name"])
