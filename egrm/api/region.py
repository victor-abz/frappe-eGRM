...
def list(project_id=None):
	...
	try:
		...
		frappe.log_error(f"Error in list_regions: {e!s}")
...
def hierarchy(region_id):
	...
	try:
		...
		if not frappe.has_permission("GRM Project", "read", region.project):
			...
			return {
				"status": "error",
				"message": _("You do not have permission to access this region's project"),
			}
		...
		frappe.log_error(f"Error in get_hierarchy: {e!s}")
		return {"status": "error", "message": str(e)}
...
def children(region_id):
	...
	try:
		...
		frappe.log_error(f"Error in get_children: {e!s}")
		return {"status": "error", "message": str(e)}
...
def top_level(project_id):
	...
	try:
		user = frappe.session.user
		frappe.log(f"Getting top-level regions for project {project_id} by user: {user}")
		...
		frappe.log(f"Returning {len(regions)} top-level regions for project {project_id}")
		return {"status": "success", "data": regions}

	except Exception as e:
		frappe.log_error(f"Error in get_top_level_regions: {e!s}")
		return {"status": "error", "message": str(e)}
