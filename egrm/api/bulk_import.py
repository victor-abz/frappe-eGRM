...
def download_workers_template(project: str | None = None) -> dict:
	...
	if project:
		_ensure_project(project)
	rows = [
		["Email", "First Name", "Last Name", "Role", "Department", "Region", "Phone"],
		[
			"worker1@example.test",
			"Worker",
			"One",
			"GRM Investigate & Resolve",
			"Health",
...
def upload_admin_regions(project: str | None = None, highest_level: str | None = None) -> dict:
		...
		for row_num, row in enumerate(reader, start=2):
			...
			try:
				...
				if role_name not in _BULK_IMPORT_ALLOWED_ROLES:
					...
					# import flow.
					frappe.throw(
						_("Role {0} is not allowed in bulk worker import. " "Allowed: {1}").format(
							role_name,
