...
def check_activation_status(email):
	...
	try:
		...
		frappe.log(f"Status check successful for {email}: {assignment_data.activation_status}")
		return {
			"success": True,
...
def bulk_send_activation_codes(project_code, filters=None):
	...
	if frappe.session.user != "Administrator" and not (roles & GRM_ALL_PROJECTS_ROLES):
		frappe.throw(
	...
	try:
		...
		for assignment in assignments:
			try:
				...
				failed_count += 1
				error_msg = f"Failed to send to {assignment.user}: {send_error!s}"
		...
		frappe.log_error(f"Bulk send error: {e!s}")
		return {
			"success": False,
