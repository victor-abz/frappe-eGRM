...
def get_user_projects():
    ...
    try:
        ...
        log.info(f"Getting projects for user: {user}")
        
        # Check if user is Administrator or has System Manager role (full access)
        if user == "Administrator" or "System Manager" in frappe.get_roles(user):
            projects = frappe.get_all("GRM Project", 
                                      fields=["name", "project_name", "description", "active"])
            log.info(f"Admin user, returning all {len(projects)} projects")
        ...
        log.info(f"Returning {len(projects)} projects for user {user}")
        return {"status": "success", "data": projects}
        
    except Exception as e:
        log.error(f"Error in get_user_projects: {str(e)}")
...
def get_system_info():
    ...
    try:
        ...
        log.error(f"Error in get_system_info: {str(e)}")
        return {"status": "error", "message": str(e)}
