...
def upload(issue_id=None, file_data=None, description=None):
    ...
    try:
        ...
        frappe.log(f"Uploading attachment for issue {issue_id} by user: {user}")
        ...
        frappe.log(f"Successfully uploaded attachment for issue {issue_id}")
        return {
            "status": "success",
        ...
        frappe.log_error(f"Error in upload_attachment: {str(e)}")
...
def list(issue_id):
    ...
    try:
        user = frappe.session.user
        frappe.log(f"Listing attachments for issue {issue_id} by user: {user}")
        ...
        frappe.log(f"Returning {len(attachments)} attachments for issue {issue_id}")
        return {"status": "success", "data": attachments}

    except Exception as e:
        frappe.log_error(f"Error in list_attachments: {str(e)}")
        return {"status": "error", "message": str(e)}
...
def download(attachment_id):
    ...
    try:
        user = frappe.session.user
        frappe.log(f"Downloading attachment {attachment_id} by user: {user}")
        ...
        if not frappe.has_permission("GRM Issue", "read", attachment.parent):
            log.warning(
                f"User {user} does not have permission to read issue {attachment.parent}"
            )
            return {
                "status": "error",
                "message": _("You do not have permission to access this attachment"),
            }
        ...
        # Return download URL
        frappe.log(f"Returning download URL for attachment {attachment_id}")
        return {"status": "success", "data": {"download_url": file_url}}

    except Exception as e:
        frappe.log_error(f"Error in download_attachment: {str(e)}")
        return {"status": "error", "message": str(e)}
...
def save_file(file_data, issue_id):
    ...
    try:
        ...
        frappe.log_error(f"Error saving file: {str(e)}")
        return None
