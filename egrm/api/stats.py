...
def dashboard(project_id=None, date_range=None):
	...
	try:
		...
		stats = {
			...
			"category_breakdown": get_category_breakdown(accessible_projects, date_filter),
			...
			"user_performance": get_user_performance(accessible_projects, user, date_filter),
		...
		frappe.log_error(f"Error in dashboard: {e!s}")
...
def user_stats(project_id=None, date_range=None):
	...
	try:
		...
		# Get user statistics
		stats = {
			"assigned_issues": get_user_assigned_issues(accessible_projects, user, date_filter),
			"reported_issues": get_user_reported_issues(accessible_projects, user, date_filter),
			"resolved_issues": get_user_resolved_issues(accessible_projects, user, date_filter),
			"performance_metrics": get_user_performance_metrics(accessible_projects, user, date_filter),
			"workload_distribution": get_user_workload_distribution(accessible_projects, user),
			"activity_timeline": get_user_activity_timeline(accessible_projects, user, date_filter),
		}
		...
		frappe.log_error(f"Error in user_stats: {e!s}")
		return {"status": "error", "message": str(e)}
...
def project_summary(project_id):
	...
	try:
		...
		frappe.log_error(f"Error in project_summary: {e!s}")
		return {"status": "error", "message": str(e)}
...
def get_overview_stats(accessible_projects, date_filter):
	...
	# Open issues
	open_statuses = frappe.get_all("GRM Issue Status", filters={"open_status": 1}, fields=["name"])
	...
	# Resolved issues
	resolved_statuses = frappe.get_all("GRM Issue Status", filters={"final_status": 1}, fields=["name"])
	...
	pending_statuses = frappe.get_all("GRM Issue Status", filters={"initial_status": 1}, fields=["name"])
...
def get_trend_data(accessible_projects, date_range):
	...
	for i in range(days):
		...
		# A dict cannot carry two conditions on `creation` — the second
		# key silently replaced the first, so every bucket counted every
		# issue created *before* the next day instead of that day's own.
		filters = [
...
def get_user_performance_metrics(accessible_projects, user, date_filter):
	...
	avg_resolution_time = calculate_avg_resolution_time(accessible_projects, user, date_filter)
...
def get_user_activity_timeline(accessible_projects, user, date_filter):
	...
	timeline = []
	for issue in issues:
		timeline.append({"date": issue.creation.strftime("%Y-%m-%d"), "type": "created", "count": 1})
...
def calculate_avg_resolution_time(accessible_projects, user, date_filter):
	...
	issues = frappe.get_all("GRM Issue", filters=filters, fields=["creation", "resolution_date"])
...
def get_project_team_members(project_id):
	...
	for assignment in assignments:
		...
		assigned_count = frappe.db.count("GRM Issue", {"project": project_id, "assignee": assignment.user})

		reported_count = frappe.db.count("GRM Issue", {"project": project_id, "reporter": assignment.user})
