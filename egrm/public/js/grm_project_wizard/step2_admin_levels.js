...
class GRMWizardStep2AdminLevelsInner {
		...
		this.$body.html(`
            <div class="grm-step3" style="max-width: 960px;">
              <div class="grm-step3-intro" style="margin-bottom: 16px;">
                <p>${__(
					"Administrative Levels are the geographic / organizational hierarchy used to route GRM cases (e.g. National &rarr; Region &rarr; District &rarr; Sector &rarr; Cell)."
				)}</p>
                <p class="text-muted small">${__(
					"Lower level_order = higher in the tree (1 = root). Each level defines its own SLA defaults: acknowledgment, resolution, reminder lead time, and auto-escalation."
				)}</p>
              </div>
              <div id="grm-step3-table-wrap"></div>
              <div id="grm-step3-form-wrap" style="margin-top: 12px;"></div>
              <div style="margin-top: 12px;">
                <button class="btn btn-default btn-sm" id="grm-step3-add">+ ${__(
					"Add Level"
				)}</button>
              </div>
            </div>
		...
		if (!this.rows.length) {
			$w.html(
				`<p class="text-muted">${__(
					'No administrative levels yet — click "Add Level" to create the first one.'
				)}</p>`
			);
			return;
		}
	...
	render_row_html(r) {
		...
		if (editing) {
			return `
              <tr data-name="${frappe.utils.escape_html(r.name)}">
                ...
                <td><input type="text" class="form-control input-xs" id="grm-e-level_name" value="${frappe.utils.escape_html(
					r.level_name || ""
				)}"></td>
                <td><input type="number" min="1" class="form-control input-xs" id="grm-e-level_order" value="${
					r.level_order != null ? r.level_order : ""
				}"></td>
                <td><input type="number" min="0" class="form-control input-xs" id="grm-e-acknowledgment_days" value="${
					r.acknowledgment_days != null ? r.acknowledgment_days : 7
				}"></td>
                <td><input type="number" min="0" class="form-control input-xs" id="grm-e-resolution_days" value="${
					r.resolution_days != null ? r.resolution_days : 30
				}"></td>
                <td><input type="number" min="0" class="form-control input-xs" id="grm-e-reminder_before_days" value="${
					r.reminder_before_days != null ? r.reminder_before_days : 2
				}"></td>
                <td><input type="checkbox" id="grm-e-auto_escalate" ${
					r.auto_escalate ? "checked" : ""
				}></td>
                <td>
                  <button class="btn btn-xs btn-primary grm-save-edit" data-name="${frappe.utils.escape_html(
						r.name
					)}">${__("Save")}</button>
		...
		return `
          <tr data-name="${frappe.utils.escape_html(r.name)}">
            ...
            <td>
              <button class="grm-row-action grm-edit" title="${__(
					"Edit"
				)}" data-name="${frappe.utils.escape_html(r.name)}">${frappe.utils.icon(
			"edit",
			"sm"
		)}</button>
              <button class="grm-row-action grm-row-action-danger grm-delete" title="${__(
					"Delete"
				)}" data-name="${frappe.utils.escape_html(r.name)}">${frappe.utils.icon(
			"close",
			"sm"
	...
	async save_edit(name) {
		...
		if (isNaN(level_order) || level_order < 1) {
			frappe.show_alert({
				message: __("Level Order must be an integer >= 1."),
				indicator: "red",
			});
			return;
		}
		...
		const dup = this.rows.find(
			(x) =>
				x.name !== name && (x.level_name || "").toLowerCase() === level_name.toLowerCase()
		);
		if (dup) {
			frappe.show_alert({
				message: __("Level Name '{0}' already exists for this project.", [level_name]),
				indicator: "red",
			});
			return;
		}
