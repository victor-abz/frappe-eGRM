...
class GRMWizardStep2AdminUnits {
	...
	render() {
		...
		this.$body.html(`
            <div class="grm-step2-admin">
              <ul class="nav nav-tabs" role="tablist">
                <li class="nav-item"><a class="nav-link active" data-toggle="tab" href="#grm-tab-levels">${__(
					"Levels"
				)}</a></li>
                <li class="nav-item"><a class="nav-link" data-toggle="tab" href="#grm-tab-regions">${__(
					"Regions"
				)}</a></li>
              </ul>
        ...
        `);
		this.levels_inner = new GRMWizardStep2AdminLevelsInner(
			this.$body.find("#grm-tab-levels"),
			this.project,
			this.wizard
		);
		this.regions_inner = new GRMWizardStep2AdminRegionsInner(
			this.$body.find("#grm-tab-regions"),
			this.project,
			this.wizard
		);
	}
