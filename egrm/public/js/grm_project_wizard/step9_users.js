class GRMWizardStep9Users {
		...
		this.$body.html(`
            ...
            <div class="grm-step9-add-section">
              <div class="grm-step9-add-toggle" role="tablist">
                <button type="button" class="btn btn-default active" data-mode="single">${__(
					"Single user"
				)}</button>
                <button type="button" class="btn btn-default" data-mode="bulk">${__(
					"CSV/Excel import"
				)}</button>
              </div>
	...
	async refresh_coverage() {
		...
		if (!gaps.length) {
			$banner.html(`
                <div class="alert alert-success" style="margin-bottom:12px;">
                  ...
                  — ${__("all {0} region(s) have Intake, Review, and Investigate & Resolve.", [
						coverage.total_regions,
					])}
                </div>
            `);
			return;
		...
		$banner.html(`
            <div class="alert alert-warning grm-cov-banner" style="margin-bottom:12px;">
              <strong>${__("{0} of {1} region(s) missing duty coverage", [
					gaps.length,
					coverage.total_regions,
				])}</strong>
              <p class="text-muted small" style="margin:4px 0;">
                ${__(
					"Each region must cover Intake, Review, and Investigate & Resolve duties. Add users below, or remove regions you don't intend to use."
				)}
              </p>
		...
		$btn.on("click", async () => {
			const ids = $banner
				.find(".grm-cov-gap-cb:checked")
				.map(function () {
					return $(this).closest("tr").data("region");
				})
				.get()
				.filter(Boolean);
			...
			try {
				...
				frappe.show_alert({
					message: __("Could not compute removal impact"),
					indicator: "red",
				});
			...
			const sample = (preview.rows || [])
				.map((row) => {
					...
					if (row.descendants)
						parts.push(__("{0} descendant region(s)", [row.descendants]));
			...
			const header = cascade_needed ? __("Confirm cascade removal") : __("Confirm removal");
			const summary = __(
			...
			const enforced = cascade_needed
				? __(
						"There is no way to keep the descendants — descendants without an ancestor produce orphan regions, which the wizard refuses to create."
				  )
				: "";
			const body_html = `
                <div>
                    ...
                    ${
						sample
							? `<ul style="margin: 8px 0; padding-left: 18px;">${sample}</ul>`
							: ""
					}
                    ${
						enforced
							? `<p style="color:#9e3a3a; margin: 6px 0 0 0;">${frappe.utils.escape_html(
									enforced
							  )}</p>`
							: ""
					}
			...
			const confirmed = await new Promise((resolve) => {
				...
				d.show();
				d.$wrapper.find(".btn-modal-secondary").on("click", () => {
					resolve(false);
				});
			});
			if (!confirmed) {
			...
			try {
				...
				if (cascaded_regions)
					msg += "; " + __("plus {0} descendant region(s)", [cascaded_regions]);
	...
	async save() {
		...
		const sample = gaps
			...
			.map(
				(g) =>
					`<li><code>${frappe.utils.escape_html(
						g.region_path || g.region_name
					)}</code>: ${g.missing_duties.join(", ")}</li>`
			)
			.join("");
		const more =
			gaps.length > 5
		...
		frappe.msgprint({
			...
			message: `
                <p>${__(
					"Each region must have at least one user covering Intake, Review, and Investigate & Resolve."
				)}</p>
                <p>${__("{0} of {1} region(s) are missing coverage:", [
					gaps.length,
					coverage.total_regions,
				])}</p>
                <ul>${sample}${more}</ul>
