...
class GRMWizardStep6NotificationTemplates {
		...
		this.$body.html(`
            <div class="grm-step11" style="max-width: 1100px;">
              <div class="grm-step11-intro" style="margin-bottom: 16px;">
                <p>${__(
					"Notification Templates control the messages sent to citizens at each stage of an issue's lifecycle (receipt, acknowledgment, status updates, escalation, SLA reminders)."
				)}</p>
                <p class="text-muted small">${__(
					"Each template can drive an email (linked Email Template) and / or an SMS (Jinja2 message body). Use {{ tracking_code }}, {{ status }}, {{ subject }}, etc. as placeholders."
				)}</p>
              </div>
              <div id="grm-step11-table-wrap"></div>
              <div id="grm-step11-form-wrap" style="margin-top: 12px;"></div>
              <div style="margin-top: 12px;">
                <button class="btn btn-default btn-sm" id="grm-step11-add">+ ${__(
					"Add Template"
				)}</button>
              </div>
            </div>
	...
	render_table() {
		...
		if (!this.rows.length) {
			$w.html(
				`<p class="text-muted">${__(
					'No notification templates yet — click "Add Template" to create the first one.'
				)}</p>`
			);
			return;
		}
		...
		const body_rows = this.rows
            ...
            <tr data-name="${frappe.utils.escape_html(r.name)}">
              ...
              <td>
                <button class="grm-row-action grm-edit-tpl" title="${__(
					"Edit"
				)}" data-name="${frappe.utils.escape_html(r.name)}">${frappe.utils.icon(
					"edit",
					"sm"
				)}</button>
                <button class="grm-row-action grm-row-action-danger grm-delete-tpl" title="${__(
					"Delete"
				)}" data-name="${frappe.utils.escape_html(r.name)}">${frappe.utils.icon(
					"close",
					"sm"
	...
	type_options(selected) {
		...
		for (const t of types) {
			...
			opts.push(
				`<option value="${frappe.utils.escape_html(t)}"${sel}>${frappe.utils.escape_html(
					t
				)}</option>`
			);
		}
		...
		for (const t of this.email_templates) {
			...
			opts.push(
				`<option value="${frappe.utils.escape_html(
					t.name
				)}"${sel}>${frappe.utils.escape_html(t.name)}</option>`
			);
		}
            ...
            <div class="grm-step11-form card" style="border:1px solid var(--border-color, #d1d8dd); padding:12px; border-radius:6px;">
              ...
              <div class="row">
                <div class="col-md-6">
                  ...
                  <input type="text" class="form-control" id="grm-tf-template_name" value="${frappe.utils.escape_html(
						r.template_name || ""
					)}" ${is_edit ? "disabled" : ""}>
                  ${
						is_edit
							? `<small class="text-muted">${__(
									"Template name is the record id and can't be changed."
							  )}</small>`
							: ""
					}
                </div>
              ...
              <div class="row" style="margin-top:8px;">
                ...
                <div class="col-md-6">
                  ...
                  <div><label class="checkbox"><input type="checkbox" id="grm-tf-active" ${
						active ? "checked" : ""
					}> ${__("Template Active")}</label></div>
                </div>
              </div>
              ...
              <div class="form-group">
                ...
                <textarea class="form-control" id="grm-tf-sms_message" rows="4" placeholder="${__(
					"e.g. Issue {{ tracking_code }} is now {{ status }}."
				)}">${frappe.utils.escape_html(r.sms_message || "")}</textarea>
                <small class="text-muted">${__(
					"Supports Jinja2: {{ tracking_code }}, {{ status }}, {{ subject }}, {{ complainant_name }}, {{ created_date }}, etc."
				)}</small>
              </div>
              <div style="margin-top:12px;">
                <button class="btn btn-primary btn-sm" id="grm-tf-save">${__(
					"Save Template"
				)}</button>
	...
	async save_form(existing_name) {
		...
		if (!existing_name) {
			...
			if (dup) {
				frappe.show_alert({
					message: __("Template '{0}' already exists for this project.", [
						v.template_name,
					]),
					indicator: "red",
				});
		...
		if (dup_type) {
			frappe.show_alert({
				message: __('A template of type "{0}" already exists for this project.', [
					v.template_type,
				]),
				indicator: "red",
			});
	...
	confirm_delete(name) {
		frappe.confirm(__("Delete notification template {0}?", [name]), async () => {
			try {
