...
class GRMWizardStep9UserImport {
	...
	_render_stage() {
		...
		switch (this.stage) {
			case "template":
				this._render_template_stage($content);
				break;
			case "upload":
				this._render_upload_stage($content);
				break;
			case "mapping":
				this._render_mapping_stage($content);
				break;
			case "preview":
				this._render_preview_stage($content);
				break;
		}
	}
	...
	_render_template_stage($content) {
		$content.html(`
            <p>${__(
				"Download a project-tailored template, fill it in with your users, then upload below."
			)}</p>
            <p class="text-muted small">${__(
				"Columns include your project's admin levels (e.g. Province / District / Sector), required user fields (email, first name, last name), and required assignment fields (role)."
			)}</p>
            <div class="btn-group" role="group">
              <button type="button" class="btn btn-default grm-template-csv">${__(
					"Download CSV template"
				)}</button>
              <button type="button" class="btn btn-default grm-template-xlsx">${__(
					"Download Excel template"
				)}</button>
            </div>
            <div class="grm-step9-add-actions">
              <button type="button" class="btn btn-primary grm-stage-next">${__(
					"Continue to upload"
				)}</button>
            </div>
        `);
	...
	_render_upload_stage($content) {
		const status_html = this.uploaded_file_url
			? `<p class="grm-upload-status text-success">${__("Uploaded: {0}", [
					frappe.utils.escape_html(this.uploaded_file_name),
			  ])}</p>`
		...
		$content.html(`
            <div class="grm-step9-upload-zone">
              ...
              <button type="button" class="btn btn-default grm-upload-browse">${__(
					"Choose file"
				)}</button>
            </div>
            ${status_html}
	...
	_render_mapping_stage($content) {
		...
		const target_options_html = (selected) =>
			target_options
				.map((t) => {
					const sel = t.value === selected ? "selected" : "";
					return `<option value="${frappe.utils.escape_html(
						t.value
					)}" ${sel}>${frappe.utils.escape_html(t.label)}</option>`;
				})
				.join("");

		const level_options_html = (selected) =>
			project_levels
				.map((l) => {
					const val = l.name;
					...
					return `<option value="${frappe.utils.escape_html(
						val
					)}" ${sel}>${frappe.utils.escape_html(l.level_name || val)}</option>`;
				})
				.join("");
		...
		const rows_html = this.headers
			.map((h) => {
				...
				const level_html = is_region
					? `
                <select class="form-control form-control-sm grm-level-select" data-header="${frappe.utils.escape_html(
					h
				)}">
            ...
            `
              ...
              <tr data-header="${frappe.utils.escape_html(h)}">
                ...
                <td>
                  <select class="form-control form-control-sm grm-target-select" data-header="${frappe.utils.escape_html(
						h
					)}">
		...
		$content.html(`
            <p>${__(
				"Map each column from your file to a target field. Required fields are marked with an asterisk."
			)} <span class="text-muted">${total_rows_label}.</span></p>
            ...
            <div class="grm-mapper-options">
              ...
              <label class="ml-3" title="${__(
					"Users will log in with their phone number. Username and mobile_no are set from the phone digits; placeholder emails are auto-generated for rows without one."
				)}">
              ...
              <label class="ml-3 grm-legacy-synth-wrap" title="${__(
					"Legacy: builds firstname.lastname@<domain> for rows without an email. Phone-as-login takes precedence when both are checked."
				)}">
            ...
            <div class="grm-step9-add-actions">
              <button type="button" class="btn btn-default grm-stage-back">${__("Back")}</button>
              <button type="button" class="btn btn-primary grm-stage-next" ${
					this.validation && this.validation.ok ? "" : "disabled"
				}>${__("Continue to preview")}</button>
            </div>
        `);
		...
		$content.on(
			"change",
			".grm-phone-as-username, .grm-synthesize-emails",
	...
	_revalidate() {
		...
		Object.entries(this.mapping || {}).forEach(([header, m]) => {
			...
			if (t === "administrative_region") {
				if (!m.level_type) {
					errors.push(
						__(
							"Column '{0}' is mapped to administrative region but has no level type selected.",
							[header]
						)
					);
				} else {
		...
		for (const [lt, headers] of level_seen.entries()) {
			if (headers.length > 1) {
				errors.push(
					__("Multiple columns mapped to admin level '{0}': {1}. Pick exactly one.", [
						lt,
						headers.join(", "),
					])
				);
	...
	_render_validation_banner() {
		...
		if (v.ok) {
			return `<div class="alert alert-success">${__(
				"Mapping is complete — ready to preview."
			)}</div>`;
		}
		const parts = [];
	...
	async _start_preview() {
		...
		const synthesize_email_domain = (this.$mount.find(".grm-synth-domain").val() || "")
			.trim()
			.toLowerCase();
		...
		if (synthesize_emails && !phone_as_username) {
			...
			!domain_re.test(synthesize_email_domain)
		) {
			frappe.msgprint({
				title: __("Invalid email domain"),
				message: __("Email domain '{0}' is not valid.", [synthesize_email_domain]),
	...
	_render_preview_stage($content) {
		...
		const regions_html =
			this.regions_to_create && this.regions_to_create.length
				? `
            <div class="alert alert-info">
              ...
              ${this.regions_to_create
					.map((r) => {
						...
						return `<code>${frappe.utils.escape_html(lt)}=${frappe.utils.escape_html(
							v
						)}</code>`;
					})
					.join(", ")}
		...
		const missing_roles_html = missing_roles.length
            ...
            <div class="alert alert-danger">
              ...
              ${missing_roles
					.map((r) => `<code>${frappe.utils.escape_html(String(r))}</code>`)
					.join(", ")}
            </div>`
			: "";
		...
		const warnings_html =
            ...
            <div class="alert alert-info">
              ${p.warnings
					.map((w) => `<div>${frappe.utils.escape_html(String(w))}</div>`)
					.join("")}
            </div>`
				: "";
		...
		const no_import_hint =
			!can_start && p.rows_ready === 0
				? `<p class="text-muted">${__(
						"No rows are ready to import. Adjust your mapping or input file and try again."
				  )}</p>`
				: "";
		...
		$content.html(`
            ...
            <div class="grm-step9-add-actions">
              <button type="button" class="btn btn-default grm-stage-back">${__("Back")}</button>
              <button type="button" class="btn btn-success grm-import-start" ${start_disabled}>${__(
			"Start import"
		)}</button>
            </div>
        `);
	...
	_poll_progress() {
		const tick = async () => {
			...
			this.$mount
				.find(".grm-progress-text")
				.text(__("{0}: {1} succeeded, {2} failed", [s.status || "", succeeded, failed]));
