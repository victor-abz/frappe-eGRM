...
class GRMWizardStep2AdminRegionsInner {
	...
	render() {
		this.$container.html(`
            <p class="text-muted">${__(
				"Upload a CSV with one column per administrative level (e.g. Province, District, Sector). The highest level is auto-created from the project's country."
			)}</p>
            ...
            <button class="btn btn-primary btn-sm" id="grm-rg-import" disabled>${__(
				"Import Regions"
			)}</button>
	...
	_render_preview() {
		...
		if (p.errors && p.errors.length) {
			$r.append(
				`<div class="alert alert-danger"><strong>${__("Errors")}:</strong><ul>${p.errors
		...
		$r.append(
			`<div class="alert alert-info">${__("Detected {0} rows across levels: {1}", [
				p.total_rows || 0,
				cols.join(" → "),
			])}</div>`
		);
		const $tbl = $(
			`<div class="form-grid"><table class="table table-borderless"><thead><tr>${cols
				.map((c) => `<th>${frappe.utils.escape_html(c)}</th>`)
				.join("")}</tr></thead><tbody></tbody></table></div>`
		);
		(p.preview || []).forEach((row) => {
			$tbl.find("tbody").append(
				`<tr>${cols
					.map((c) => `<td>${frappe.utils.escape_html(row[c] || "")}</td>`)
					.join("")}</tr>`
			);
		});
		$r.append($tbl);
	...
	async do_import() {
		...
		$r.append(
			`<div class="alert alert-success">${__(
				"Imported {0} regions ({1} updated). {2} errors.",
		...
		if (m.errors && m.errors.length) {
			$r.append(
				`<ul>${m.errors
					.map((e) => `<li class="text-danger">${frappe.utils.escape_html(e)}</li>`)
					.join("")}</ul>`
			);
		}
	}
