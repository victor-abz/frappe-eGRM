...
class GRMWizardStep13Activate {
		...
		this.$body.html(`
            <div class="grm-step12" style="max-width: 720px;">
              <div class="grm-summary-card" style="border:1px solid var(--border-color, #d1d8dd); border-radius:6px; padding:16px; margin-bottom:16px;">
                ...
                <div id="grm-step12-summary"><p class="text-muted">${__(
					"Loading counts..."
				)}</p></div>
              </div>
              <div class="grm-coverage-card" style="border:1px solid var(--border-color, #d1d8dd); border-radius:6px; padding:16px; margin-bottom:16px;">
                ...
                <p class="text-muted small" style="margin-bottom:8px;">${__(
					"Each region must have at least one user covering Intake, Review, and Investigate & Resolve. Duties may be split across multiple users or held by one person."
				)}</p>
                <div id="grm-step12-coverage"><p class="text-muted">${__(
					"Loading coverage..."
				)}</p></div>
              </div>
	...
	render_coverage(coverage) {
		...
		const rows = gaps
            ...
            <tr>
              ...
              <td>${g.missing_duties
					.map(
						(d) =>
							`<span class="indicator-pill orange">${frappe.utils.escape_html(
								d
							)}</span>`
					)
					.join(" ")}</td>
		...
		const more =
			gaps.length > 50
				? `<p class="text-muted small">${__("Showing 50 of {0} gaps.", [gaps.length])}</p>`
				: "";
		$c.html(`
            <div class="alert alert-warning" style="margin-bottom:8px;">
              ...
              ${__(
					"Activation is blocked until every region has Intake, Review, and Investigate & Resolve."
				)}
            </div>
		...
		$s.html(`
            <div class="form-grid">
              <table class="table table-borderless" style="margin-bottom:0;">
                <tbody>
                  <tr><th style="width:40%;">${__(
						"Project Code"
					)}</th><td>${frappe.utils.escape_html(p.project_code || "")}</td></tr>
                  <tr><th>${__("Title")}</th><td>${frappe.utils.escape_html(
			p.title || ""
		)}</td></tr>
		...
		if (already) {
			$a.append(`
                <div class="alert alert-success" style="margin-bottom:0;">
                  ...
                  <a href="/app/grm-project/${encodeURIComponent(p.name)}" class="ml-2">${__(
				"Open project record"
			)}</a>
                </div>
            `);
			...
			$a.append(`
                <p>${__(
					'Tick the confirmation above, then click "Activate Project" to mark setup complete and switch to the Platform workspace.'
				)}</p>
            `);
