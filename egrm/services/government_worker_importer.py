...
class OptimizedBulkWorkerCreator:
    ...
    def _validate_inputs(self) -> None:
        try:
            ...
            # nothing to bind to.
            duty_count = (
                frappe.qb.from_("GRM Duty").select("name").run()
            )
            if not duty_count:
                raise ValueError(
    ...
    def generate_for_regions(self, level_filter: str | None = None, position_template: str = "{level}_officer") -> dict:
    ...
    def export_activation_codes_csv(self) -> str:
        ...
        writer = csv.DictWriter(
            buf,
            fieldnames=[
                "email", "username", "activation_code", "status",
                "position", "region", "department",
                "expires_on", "activated_on", "code_sent_on",
            ],
        )
        writer.writeheader()
        for r in rows:
    ...
    def _wizard_rows_to_worker_data(self, rows: list[dict]) -> tuple[list[dict], list[str]]:
        ...
        for r in rows:
            ...
            if not region_id:
                errors.append(f"Row {r['_row_num']}: region '{region_name}' not found in project {self.project_code}")
                continue
            position = r["position"]
    ...
    # ------------------------------------------------------------------
    def _generate_worker_data_for_region(self, region: dict, position_template: str = "{level}_officer") -> dict:
        ...
        project_role_link = (
            self._project_role_link(DEFAULT_PROJECT_ROLE_NAME)
            or self._first_project_role_link()
        )
        return {
    ...
    def _bulk_insert_users_sql(self, user_data_list: list[dict]) -> None:
        ...
        for i in range(0, len(user_data_list), self.batch_size):
            ...
            if valid_users:
                values = [
                    (
                        u["name"], u["username"], u["email"], u["first_name"],
                        u["last_name"], u["full_name"], u["enabled"],
                        u["send_welcome_email"], u["creation"], u["modified"],
                        u["owner"], u["modified_by"], u["docstatus"],
                    )
    ...
    def _bulk_insert_assignments_sql(self, assignment_data_list: list[dict]) -> None:
        for i in range(0, len(assignment_data_list), self.batch_size):
            ...
            values = [
                (
                    a["name"], a["user"], a["project"], a["role"], a["position_title"],
                    a["administrative_region"], a["department"], a["is_active"],
                    a["activation_code"], a["activation_status"], a["activation_expires_on"],
                    a["creation"], a["modified"], a["owner"], a["modified_by"], a["docstatus"],
                )
                for a in valid
            ]
            placeholders = ", ".join(["%s"] * len(values[0]))
    ...
    def _fetch_activation_codes(self) -> list[dict]:
        ...
        return (
            frappe.qb.from_(a)
            .left_join(u).on(a.user == u.name)
            .left_join(r).on(a.administrative_region == r.name)
            .select(
                u.email, u.username, a.activation_code, a.activation_status,
                a.position_title, r.region_name, a.department,
                a.activation_expires_on, a.activated_on, a.code_sent_on,
            )
    ...
    def _create_from_legacy_stream(self, fh) -> bool:
        ...
        for row_num, row in enumerate(reader, start=2):
            if (row.get("region_id", "").startswith("#")
