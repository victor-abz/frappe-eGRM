...
def change_status(issue, status, comment):
    ...
    try:
        ...
        log.error(f"Error changing issue status: {str(e)}")
...
def escalate_issue(issue, reason, due_at):
    ...
    try:
        ...
        doc.append("grm_issue_escalation_reason", {
        ...
        if category_doc.assigned_escalation_department:
            ...
            if dept_head:
                ...
                # Add log entry
                log_text = _("Issue assigned to escalation department head: {0}").format(dept_head)
                doc.append("grm_issue_log", {
                    "text": log_text,
                    "user": frappe.session.user,
                    "timestamp": now_datetime()
        ...
        log.error(f"Error escalating issue: {str(e)}")
...
def reassign_issue(issue, assignee, comment):
    ...
    try:
        ...
        log.error(f"Error reassigning issue: {str(e)}")
...
def collect_feedback(issue, resolution_accepted, rating, comment):
    ...
    try:
        ...
        log.error(f"Error collecting feedback: {str(e)}")
...
def is_valid_status(project, status):
    ...
    try:
        ...
        log.error(f"Error checking valid status: {str(e)}")
        return False
...
def is_final_status(status):
    ...
    try:
        ...
        log.error(f"Error checking final status: {str(e)}")
        return False
...
def has_project_access(user, project):
    ...
    try:
        ...
        log.error(f"Error checking project access: {str(e)}")
        return False
...
def add_comment_and_log(doc, comment, log_text):
    ...
    try:
        ...
        log.error(f"Error adding comment and log: {str(e)}")
        raise
