...
def check_issue_escalations():
    ...
    try:
        ...
        frappe.log_error(f"Error in scheduled check for issue escalations: {str(e)}")
...
def process_project_escalations(project, escalation_days):
    ...
    try:
        ...
        frappe.log(
            f"Found {len(issues)} issues needing escalation for project {project}"
        )
        ...
        for issue in issues:
            try:
                ...
                frappe.log_error(
                    f"Error processing escalation for issue {issue.name}: {str(e)}"
                )
    except Exception as e:
        frappe.log_error(
            f"Error processing escalations for project {project}: {str(e)}"
        )
