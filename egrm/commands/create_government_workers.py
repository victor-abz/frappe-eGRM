...
@click.option("--with-examples", is_flag=True, help="Include example worker data in the template")
@pass_context
def export_regions_template(context, project_code, level=None, output_file=None, with_examples=False):
	"""
	...
	try:
		...
		if level:
			regions_query = regions_query.where(frappe.qb.Field("administrative_level") == level)
		...
		if not output_file:
			level_suffix = f"_{level.lower().replace(' ', '_')}" if level else "_all_levels"
		...
		role_options = ", ".join(
			[role for role in government_roles if "Field Officer" in role or "Department Head" in role]
		)
		...
		if parent_region_ids:
			...
			parent_region_names = {pr["name"]: pr["region_name"] for pr in parent_regions}

			# Get grandparent regions (for cells, this would be districts)
			grandparent_region_ids = [pr["parent_region"] for pr in parent_regions if pr["parent_region"]]
			...
			if grandparent_region_ids:
				...
				grandparent_region_names = {gpr["name"]: gpr["region_name"] for gpr in grandparent_regions}
		...
		with open(output_file, "w", newline="", encoding="utf-8") as csvfile:
			...
			for region in regions:
				...
				if region["administrative_level"] == "Cell" and region["parent_region"]:
					parent_info = next(
						(pr for pr in parent_regions if pr["name"] == region["parent_region"]),
						None,
					)
					if parent_info and parent_info["parent_region"]:
						region["grandparent_region_name"] = grandparent_region_names.get(
				...
				if with_examples:
					...
					example_workers = _get_example_workers_for_level(region["administrative_level"])
		...
		click.echo(f"📝 Template includes {len(regions)} regions at {level or 'all'} level(s)")
		click.echo("🔧 Edit the file to add worker details, then use create-government-workers --csv-file")

	except Exception as e:
		click.echo(f"❌ Error: {e!s}")
		frappe.log_error(f"Export template command failed: {e!s}")
	finally:
		frappe.destroy()
...
from egrm.services.government_worker_importer import OptimizedBulkWorkerCreator
...
def create_government_workers(
	...
	try:
		...
		frappe.log(f"Starting optimized government worker creation for project {project_code}")
		...
		with OptimizedBulkWorkerCreator(
			...
			if success and not dry_run:
				...
				if hasattr(creator, "skipped_users") and hasattr(creator, "skipped_assignments"):
					click.echo(
		...
		if not dry_run:
			frappe.db.rollback()
		click.echo(f"❌ Error: {e!s}")
		frappe.log_error(f"Command failed: {e!s}")
	finally:
		frappe.destroy()
...
def auto_generate_regional_workers(
	...
	try:
		...
		if send_emails:
			click.echo("⚠️  Email sending is disabled for auto-generation to prevent spam")
		...
		with OptimizedBulkWorkerCreator(
			...
			if success and not dry_run:
				...
				if hasattr(generator, "skipped_users") and hasattr(generator, "skipped_assignments"):
					click.echo(
		...
		if not dry_run:
			frappe.db.rollback()
		click.echo(f"❌ Error: {e!s}")
		frappe.log_error(f"Auto-generation command failed: {e!s}")
	finally:
		frappe.destroy()
...
@click.option(
	...
	type=click.Choice(["Draft", "Pending Activation", "Activated", "Expired", "Suspended"]),
	default=None,
)
@pass_context
def export_activation_codes(context, project_code, output_file=None, status_filter=None):
	"""
	...
	try:
		...
		query = (
			...
			.where(assignment_table.role.isin(["GRM Field Officer", "GRM Department Head"]))
		)
		...
		# Generate output filename if not provided
		if not output_file:
			status_suffix = f"_{status_filter.lower().replace(' ', '_')}" if status_filter else ""
		...
		click.echo(f"❌ Error: {e!s}")
		frappe.log_error(f"Export command failed: {e!s}")
	finally:
		frappe.destroy()
...
def generate_worker_template(context, project_code, output_file):
	...
	"""
	try:
		click.echo("⚠️  This is a legacy command. Consider using 'export-regions-template' instead.")
		...
		click.echo("🔧 Edit the file and run create-government-workers with --csv-file option")
