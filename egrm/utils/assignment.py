...
def get_department_workers(department_id, region_id):
    ...
    try:
        ...
        users = frappe.get_all(
            "Has Role", filters={"role": ["in", roles]}, fields=["parent"]
        )
        ...
        frappe.log(
            f"Found {len(assigned_users)} workers for department {department_id} in region {region_id}"
        )
        return assigned_users

    except Exception as e:
        frappe.log_error(f"Error in get_department_workers: {str(e)}")
        return []
...
def count_open_assignments(user_id):
    ...
    try:
        ...
        count = frappe.db.count(
            "GRM Issue", {"assignee": user_id, "status": ["in", open_statuses]}
        )

        frappe.log(f"User {user_id} has {count} open issues")
        return count

    except Exception as e:
        frappe.log_error(f"Error in count_open_assignments: {str(e)}")
        return 0
...
def get_open_statuses():
    ...
    try:
        ...
        frappe.log_error(f"Error in get_open_statuses: {str(e)}")
        return []
...
def select_assignee_by_workload(department_id, region_id):
    ...
    try:
        ...
        if not workers:
            log.warning(
                f"No workers found for department {department_id} in region {region_id}"
            )
            return None
        ...
        lowest_workload_workers = [
            w[0] for w in sorted_workers if w[1] == lowest_workload
        ]

        # Randomly select one of the workers with lowest workload
        selected_worker = random.choice(lowest_workload_workers)

        frappe.log(f"Selected worker {selected_worker} with workload {lowest_workload}")
        return selected_worker

    except Exception as e:
        frappe.log_error(f"Error in select_assignee_by_workload: {str(e)}")
        return None
...
def get_department_head(department_id, region_id):
    ...
    try:
        ...
        users = frappe.get_all(
            "Has Role", filters={"role": "GRM Department Head"}, fields=["parent"]
        )
        ...
        if not assigned_users:
            log.warning(
                f"No department head found for department {department_id} in region {region_id}"
            )
            return None
        ...
        frappe.log(
            f"Selected department head {selected_head} for department {department_id} in region {region_id}"
        )
        return selected_head

    except Exception as e:
        frappe.log_error(f"Error in get_department_head: {str(e)}")
        return None
...
def handle_escalation_assignment(issue_id):
    ...
    try:
        ...
        if not region.parent_region:
            frappe.log(
                f"Region {region_id} has no parent, assigning to department head"
            )
        ...
        assignee = select_assignee_by_workload(
            issue.category.department, parent_region_id
        )
        if assignee:
            frappe.log(
                f"Selected assignee {assignee} in parent region {parent_region_id}"
            )
            return assignee

        # If no assignee found, try department head
        frappe.log(f"No assignee found in parent region, trying department head")
        return get_department_head(issue.category.department, parent_region_id)

    except Exception as e:
        frappe.log_error(f"Error in handle_escalation_assignment: {str(e)}")
        return None
...
def auto_assign_issue(issue_id):
    ...
    try:
        ...
        if not category.auto_assign:
            frappe.log(f"Auto-assignment is disabled for category {category.name}")
            return None
        ...
        if issue.escalate_flag:
            frappe.log(f"Issue {issue_id} is escalated, handling escalation assignment")
        ...
        if category.assign_to_head:
            frappe.log(f"Category {category.name} is set to assign to department head")
            return get_department_head(department_id, region_id)
        else:
            frappe.log(f"Selecting assignee by workload for category {category.name}")
            return select_assignee_by_workload(department_id, region_id)

    except Exception as e:
        frappe.log_error(f"Error in auto_assign_issue: {str(e)}")
        return None
