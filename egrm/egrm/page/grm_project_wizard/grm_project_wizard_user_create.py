...
def create_assignment(
    ...
    if frappe.db.exists(
        ...
        frappe.throw(
            _("User {0} is already assigned to {1}").format(user, project)
        )
    ...
    role_project, role_admin_level = frappe.db.get_value(
        "GRM Project Role", role, ["project", "admin_level"]
    )
    if role_project != project:
        frappe.throw(
            _("Role {0} does not belong to project {1}").format(role, project)
        )
    ...
    if region:
        if not frappe.db.exists("GRM Administrative Region", region):
            frappe.throw(
                _("Administrative Region {0} does not exist").format(region)
            )
        ...
        if region_project != project:
            frappe.throw(
                _("Region {0} does not belong to project {1}").format(
                    region, project
                )
            )
        if role_admin_level:
            ...
            if (
                ...
                frappe.throw(
                    _(
                        "Region {0} sits below the role's allowed admin level"
                    ).format(region)
                )
    ...
    logger.info(
        ...
        project, user, role, region, doc.name,
    )
    return {
        "name": doc.name,
