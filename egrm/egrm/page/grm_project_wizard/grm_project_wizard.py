...
def _ensure_default_catalog(project: str) -> None:
	...
	for flag, (default_name, needed) in fill_map.items():
		...
		# row instead.
		if _has_project_link("GRM Issue Status", "status_name", default_name, project):
	...
	if not _has_project_link("GRM Issue Department", "department_name", _DEFAULT_DEPARTMENT, project):
...
def update_category_routing(project: str, category: str, target_type: str, target: str) -> dict:
	...
	_require_wizard_role()
	if target_type != "Role":
		frappe.throw(frappe._("Only Role routing is supported. Update the category to route to a Role."))
