...
class GRMIssueCategory(Document):
    def validate(self):
        try:
            ...
            log.info(f"Validating GRM Issue Category {self.name}")
        except Exception as e:
            log.error(f"Error validating GRM Issue Category: {str(e)}")
            raise
    ...
    def validate_project_links(self):
        try:
            ...
            log.error(f"Error validating project links: {str(e)}")
            raise
        ...
        try:
            ...
            if self.assigned_appeal_department:
                dept_projects = frappe.db.sql("""
                    SELECT project FROM `tabGRM Project Link`
                    WHERE parent = %s
                """, (self.assigned_appeal_department), as_dict=1)
                
                dept_project_list = [d.project for d in dept_projects]
                
                for project in project_links:
                    if project not in dept_project_list:
                        frappe.throw(_("Assigned appeal department {0} is not linked to project {1}").format(
            ...
            if self.assigned_escalation_department:
                dept_projects = frappe.db.sql("""
                    SELECT project FROM `tabGRM Project Link`
                    WHERE parent = %s
                """, (self.assigned_escalation_department), as_dict=1)
                
                dept_project_list = [d.project for d in dept_projects]
                
                for project in project_links:
                    if project not in dept_project_list:
                        frappe.throw(_("Assigned escalation department {0} is not linked to project {1}").format(
            ...
            if self.administrative_level:
                for project in project_links:
                    admin_level_exists = frappe.db.exists(
                        ...
                        {
                            "name": self.administrative_level,
                            "project": project
                        }
                    )
                    
                    if not admin_level_exists:
                        frappe.throw(_("Administrative level {0} is not linked to project {1}").format(
                            self.administrative_level, project))
        except Exception as e:
            log.error(f"Error validating departments: {str(e)}")
            raise