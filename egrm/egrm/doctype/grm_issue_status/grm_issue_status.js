frappe.ui.form.on('GRM Issue Status', {
    refresh: function(frm) {
        ...
        if (frm.doc.initial_status) {
            frm.set_intro(__('Initial status should be unique for each project.'), 'blue');
        }
    ...
    initial_status: function(frm) {
        // Check for other initial statuses in the same projects
        if (frm.doc.initial_status && frm.doc.grm_project_link && frm.doc.grm_project_link.length > 0) {
                        ...
                        frappe.call({
                            ...
                            callback: function(r2) {
                                if (r2.message && r2.message.length > 0) {
                                    ...
                                    let message = __('The following projects already have initial statuses:') + '<br>';
                                    for (let project in project_statuses) {
                                        message += `- ${project}: ${project_statuses[project].join(', ')}<br>`;
                                    }
