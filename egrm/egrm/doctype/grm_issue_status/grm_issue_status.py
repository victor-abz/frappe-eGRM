...
class GRMIssueStatus(Document):
    def validate(self):
        try:
            ...
            log.info(f"Validating GRM Issue Status {self.name}")
        except Exception as e:
            log.error(f"Error validating GRM Issue Status: {str(e)}")
            raise
            ...
            for link in self.grm_project_link:
                if self.initial_status:
                    initial_statuses = frappe.db.sql("""
                        ...
                        AND p.project = %s
                        AND s.name != %s
                    """, (link.project, self.name), as_dict=1)
                    
                    if initial_statuses:
                        frappe.throw(_("Project {0} already has an initial status: {1}").format(
                            link.project, initial_statuses[0].name))
        except Exception as e:
            log.error(f"Error validating unique status type: {str(e)}")
            raise
