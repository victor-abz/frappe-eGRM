...
class GRMProject(Document):
	def validate(self):
		try:
			...
			frappe.log_error(f"Error validating GRM Project: {e!s}")
			raise
	...
	def validate_dates(self):
		try:
			...
			frappe.log_error(f"Error validating dates for GRM Project: {e!s}")
			raise
	...
	def after_insert(self):
		try:
			...
			if not getattr(self.flags, "seed_default_config", False):
				frappe.log(f"Skipping default seed for project {self.name} " "(wizard owns its catalog).")
				return
			...
			frappe.log_error(f"Error creating default configurations for project {self.name}: {e!s}")
			frappe.throw(_("Error creating default configurations. Please check the logs."))
	...
	def create_default_statuses(self):
		try:
			...
			for status in default_statuses:
				...
				doc.insert()
				frappe.log(f"Created default status {status['status_name']} for project {self.name}")
		except Exception as e:
			frappe.log_error(f"Error creating default statuses: {e!s}")
			raise
	...
	def create_default_issue_types(self):
		try:
			...
			for issue_type in default_types:
				...
				doc.append("grm_project_link", {"project": self.name})
				doc.insert()
				frappe.log(f"Created default issue type {issue_type['type_name']} for project {self.name}")
		except Exception as e:
			frappe.log_error(f"Error creating default issue types: {e!s}")
			raise
	...
	def create_default_departments(self):
		try:
			...
			frappe.log_error(f"Error creating default department: {e!s}")
			raise
