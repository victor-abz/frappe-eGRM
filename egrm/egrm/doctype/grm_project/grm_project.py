...
class GRMProject(Document):
    ...
    def after_insert(self):
        try:
            ...
            log.error(f"Error creating default configurations for project {self.name}: {str(e)}")
    ...
    def create_default_statuses(self):
        try:
            ...
            default_statuses = [
                {"status_name": "Open", "initial_status": 1, "open_status": 1},
            ...
            for status in default_statuses:
                if not frappe.db.exists("GRM Issue Status", {"status_name": status["status_name"]}):
                    doc = frappe.new_doc("GRM Issue Status")
                    doc.update(status)
                    doc.append("grm_project_link", {"project": self.name})
                    doc.insert()
                    log.info(f"Created default status {status['status_name']} for project {self.name}")
        except Exception as e:
    ...
    def create_default_issue_types(self):
        try:
            ...
            for issue_type in default_types:
                if not frappe.db.exists("GRM Issue Type", {"type_name": issue_type["type_name"]}):
                    doc = frappe.new_doc("GRM Issue Type")
                    doc.update(issue_type)
                    doc.append("grm_project_link", {"project": self.name})
                    doc.insert()
                    log.info(f"Created default issue type {issue_type['type_name']} for project {self.name}")
        except Exception as e:
    ...
    def create_default_departments(self):
        try:
            ...
            if not frappe.db.exists("GRM Issue Department", {"department_name": "General"}):
