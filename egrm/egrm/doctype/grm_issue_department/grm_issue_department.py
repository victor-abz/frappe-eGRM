...
class GRMIssueDepartment(Document):
    def validate(self):
        try:
            ...
            log.info(f"Validating GRM Issue Department {self.name}")
        except Exception as e:
            log.error(f"Error validating GRM Issue Department: {str(e)}")
            raise
    ...
    def validate_head(self):
        try:
            ...
            log.error(f"Error validating department head: {str(e)}")
            raise
