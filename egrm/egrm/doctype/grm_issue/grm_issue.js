frappe.ui.form.on("GRM Issue", {
	...
	refresh: function (frm) {
		...
		if (frm.doc.sla_acknowledgment_status) {
			let color = { "On Time": "green", "Nearing Due": "orange", Breached: "red" }[
				frm.doc.sla_acknowledgment_status
			];
			if (color)
				frm.get_field("sla_acknowledgment_status")
					.$wrapper.find(".like-disabled-input")
					.css("color", color);
		}
		if (frm.doc.sla_resolution_status) {
			let color = { "On Time": "green", "Nearing Due": "orange", Breached: "red" }[
				frm.doc.sla_resolution_status
			];
			if (color)
				frm.get_field("sla_resolution_status")
					.$wrapper.find(".like-disabled-input")
					.css("color", color);
		}
	},
