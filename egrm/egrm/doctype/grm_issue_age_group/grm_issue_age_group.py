...
class GRMIssueAgeGroup(Document):
    def validate(self):
        try:
            self.validate_project_links()
            log.info(f"Validating GRM Issue Age Group {self.name}")
        except Exception as e:
            log.error(f"Error validating GRM Issue Age Group: {str(e)}")
            raise
    ...
    def validate_project_links(self):
        try:
            ...
            if not self.grm_project_link or len(self.grm_project_link) == 0:
                frappe.throw(_("At least one project must be linked to the age group"))
