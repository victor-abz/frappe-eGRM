...
def _sibling_gov_worker_assignments(user: str, project: str, exclude_assignment: str | None = None) -> list:
...
def _other_active_assignments_grant_duty(user: str, duty: str, exclude_assignment: str | None = None) -> bool:
...
class GRMUserProjectAssignment(Document):
	def validate(self):
		try:
			...
			frappe.log_error(f"Error validating GRM User Project Assignment: {e!s}")
			raise
	...
	def validate_creator_permissions(self):
		...
		frappe.throw(
			frappe._("You can only assign users to projects where you hold a Supervise duty."),
	...
	def validate_user(self):
		try:
			...
			frappe.log_error(f"Error validating user: {e!s}")
			raise
	...
	def validate_role(self):
		...
		try:
			...
			if not frappe.db.exists("GRM Project Role", self.role):
				frappe.throw(_("Project Role {0} does not exist").format(self.role))
			...
			if role_project != self.project:
				frappe.throw(
					_("Project Role {0} belongs to project {1}, not {2}").format(
						self.role, role_project, self.project
					)
				)
			if not role_active:
				frappe.throw(_("Project Role {0} is not active").format(self.role))
		except Exception as e:
			frappe.log_error(f"Error validating role: {e!s}")
			raise
	...
	def validate_department_and_region(self):
		...
		try:
			...
			if duties & GOVERNMENT_WORKER_DUTIES and not (self.administrative_region or self.department):
				frappe.throw(
					_(
			...
			if self.department:
				...
				if not dept_linked_to_project:
					frappe.throw(
						_("Department {0} is not linked to project {1}").format(self.department, self.project)
					)
			...
			frappe.log_error(f"Error validating department and region: {e!s}")
			raise
	...
	def validate_unique_assignment(self):
		try:
			...
			if existing:
				frappe.throw(
					_("User {0} is already assigned to project {1} with role {2} in region {3}").format(
						self.user,
						self.project,
						self.role,
			...
			frappe.log_error(f"Error validating unique assignment: {e!s}")
			raise
	...
	def handle_role_change(self, old_role):
		...
		try:
			...
			frappe.log_error(f"Error handling role change: {e!s}")
			raise
	...
	def on_update(self):
		...
		try:
			...
			frappe.log_error(f"Error in on_update: {e!s}")
			raise
	...
	def after_insert(self):
		...
		try:
			...
			frappe.log_error(f"Error in after_insert: {e!s}")
			raise
		# try:
	...
	def on_trash(self):
		try:
			...
			frappe.log(f"Removed assignment for user {self.user} for project {self.project}")
			revoke_project_access(self.user, self.project, exclude_assignment=self.name)
		except Exception as e:
			frappe.log_error(f"Error removing assignment: {e!s}")
	...
	def before_insert(self):
		...
		try:
			...
			if not self.activation_code:
				...
				frappe.log(f"Generated activation code for government worker {self.user}")
	...
	def inherit_project_activation(self) -> bool:
		...
		if any(s.activation_status == "Activated" for s in siblings):
			...
			frappe.log(f"Inherited activated state for {self.user} on {self.project}")
			return True
		...
		if any(s.activation_status == "Suspended" for s in siblings):
			...
			frappe.log(f"Inherited suspended state for {self.user} on {self.project}")
			return True
		...
		if live:
			...
			self.activation_status = "Pending Activation"
			frappe.log(f"Reused pending activation code for {self.user} on {self.project}")
			return True
	...
	def generate_activation_code(self):
		...
		try:
			...
			frappe.log_error(f"Error generating activation code: {e!s}")
			raise
	...
	def send_activation_email(self):
		...
		try:
			...
			try:
				...
				# Missing "GRM Government Worker Activation" template, or a
				# render error in it — fall back to the inline template.
				message = f"""
			...
			frappe.log_error(f"Error sending activation email: {e!s}")
	...
	def activate_worker(self, activation_code, new_password=None):
		...
		try:
			...
			if self.activation_code != activation_code:
				...
				self.save()
				frappe.throw(
					_(f"Invalid activation code. Attempts remaining: {5 - self.activation_attempts}")
				)
			...
			frappe.log_error(f"Error activating worker: {e!s}")
			raise
	...
	def cascade_activation(self) -> list[str]:
		...
		"""
		activated: list[str] = []
		for sibling in _sibling_gov_worker_assignments(self.user, self.project, exclude_assignment=self.name):
			...
			try:
				...
				frappe.log_error(f"Error cascading activation to {sibling.name}: {e!s}")
		return activated
	...
	def resend_activation_code(self):
		...
		try:
			...
			frappe.log_error(f"Error resending activation code: {e!s}")
			raise
	...
	def propagate_activation_code(self) -> list[str]:
		...
		for sibling in _sibling_gov_worker_assignments(self.user, self.project, exclude_assignment=self.name):
			...
			try:
				...
				frappe.log_error(f"Error propagating activation code to {sibling.name}: {e!s}")
		return updated
	...
	def expire_activation_code(self):
		...
		try:
			...
			frappe.log_error(f"Error expiring activation code: {e!s}")
			raise
	...
	def validate_activation_status(self):
		...
		try:
			...
			if self.activation_attempts >= 5 and self.activation_status not in [
				...
				self.activation_status = "Suspended"
				frappe.log(f"Auto-suspended account for {self.user} due to too many failed attempts")

		except Exception as e:
			frappe.log_error(f"Error validating activation status: {e!s}")
			raise
	...
	def export_activation_codes(self):
		...
		try:
			...
			government_worker_roles = _government_worker_role_names_for_project(self.project)
			...
			frappe.log_error(f"Error exporting activation codes: {e!s}")
...
def export_project_activation_codes(project_code):
	...
	try:
		...
		# Get all government worker assignments for this project
		government_worker_roles = _government_worker_role_names_for_project(project_code)
		...
		frappe.log_error(f"Error exporting project activation codes: {e!s}")
		frappe.throw(_("Error exporting activation codes: {0}").format(str(e)))
