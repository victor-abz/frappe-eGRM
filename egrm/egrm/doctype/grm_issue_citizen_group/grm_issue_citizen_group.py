...
class GRMIssueCitizenGroup(Document):
    def validate(self):
        try:
            ...
            log.info(f"Validating GRM Issue Citizen Group {self.name}")
        except Exception as e:
            log.error(f"Error validating GRM Issue Citizen Group: {str(e)}")
            raise
    ...
    def validate_project_links(self):
        try:
            ...
            if not self.grm_project_link or len(self.grm_project_link) == 0:
                frappe.throw(_("At least one project must be linked to the citizen group"))
