...
def test_parse_admin_regions_csv_returns_preview(sample_project):
    ...
    result = parse_admin_regions_csv(
        project=sample_project, highest_level="Country", csv_text=csv_text
    )
...
def test_bulk_insert_admin_regions_creates_levels_and_regions(sample_project):
    ...
    result = bulk_insert_admin_regions(
        project=sample_project, highest_level="Country", csv_text=csv_text
    )
    assert result["created"] >= 3  # Country (auto), Kigali, Gasabo
    levels = frappe.get_all(
        "GRM Administrative Level Type", filters={"project": sample_project}
    )
    assert len(levels) >= 3
