...
def _slug(value: str) -> str:
    ...
    return "".join(
        c for c in norm.encode("ascii", "ignore").decode("ascii").lower() if c.isalnum()
    ) or "user"
...
class _LargeListFixture:
    ...
    def seed(cls) -> None:
        ...
        for i in range(NUM_USERS):
            ...
            status_cycle = [
                "Draft", "Pending Activation", "Activated", "Suspended", "Expired",
            ]
    ...
    def teardown(cls) -> None:
        ...
        for level in frappe.get_all(
            "GRM Administrative Level Type",
            filters={"project": PROJECT_CODE}, pluck="name",
        ):
