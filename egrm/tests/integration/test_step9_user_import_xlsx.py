...
class _XlsxFixture:
    ...
    def teardown(cls) -> None:
        ...
        for role in frappe.get_all(
            "GRM Project Role", filters={"project": PROJECT_CODE}, pluck="name",
        ):
...
class Step9XlsxImportTests(FrappeTestCase):
    ...
    def test_xlsx_import_creates_24_users_and_resolves_regions(self) -> None:
        ...
        regions_before = frappe.db.count(
            "GRM Administrative Region", {"project": PROJECT_CODE},
        )
        self.assertEqual(
            regions_before, 0,
        ...
        if status.get("failed"):
            ...
            with open(_get_staged_path(prepared["data_import"]), "r") as fh:
        ...
        assignment_count = frappe.db.count(
            "GRM User Project Assignment", {"project": PROJECT_CODE},
        )
