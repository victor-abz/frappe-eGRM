...
class TestUnifyActivationCodesPatch(FrappeTestCase):
	...
	def setUpClass(cls) -> None:
		...
		duties = [{"duty": d} for d in ("Intake", "Investigate & Resolve") if frappe.db.exists("GRM Duty", d)]
		role = frappe.get_doc(
			{
	...
	def _teardown(cls) -> None:
		...
		for dt in (
			...
			for name in frappe.get_all(dt, filters={"project": PROJECT_CODE}, pluck="name"):
	...
	def _force(self, name: str, **values) -> None:
		...
		frappe.db.set_value("GRM User Project Assignment", name, values, update_modified=False)
	...
	def test_single_assignment_user_is_untouched(self):
		...
		lapsed = add_to_date(now(), hours=-72)
		self._force(name, activation_code="555555", activation_expires_on=lapsed, activation_status="Expired")
	...
	def test_rerunning_the_patch_changes_nothing(self):
		...
		for i, name in enumerate(names):
			self._force(
				name,
				activation_code=f"8888{i:02d}",
				activation_expires_on=lapsed,
				activation_status="Expired",
			)

		run_patch()
		first = {r.name: (r.activation_code, r.activation_status) for r in self._rows(email)}

		run_patch()
		second = {r.name: (r.activation_code, r.activation_status) for r in self._rows(email)}
