...
def execute():  # type: ignore[no-untyped-def]
	...
	for row in rows:
		...
		if int(row.enabled or 0) == should_be_enabled:
			continue
		frappe.db.set_value("Language", row.name, "enabled", should_be_enabled, update_modified=False)
		if should_be_enabled:
			enabled_count += 1
		else:
			disabled_count += 1

	missing = [code for code in SUPPORTED_LANGUAGES if not frappe.db.exists("Language", code)]
	if missing:
		...
		print("restrict_enabled_languages: WARNING — expected Language rows " f"absent: {missing}")
