...
def execute():
	if not frappe.db.exists("Role", PLATFORM_ADMIN_ROLE):
		...
		print(f"[grant_role_read_to_platform_admin] {PLATFORM_ADMIN_ROLE} " "missing — skip")
		return
	...
	if existing:
		print(f"[grant_role_read_to_platform_admin] already granted " f"({existing}) — skip")
		return
	...
	print(
		f"[grant_role_read_to_platform_admin] granted read on " f"{TARGET_DOCTYPE} to {PLATFORM_ADMIN_ROLE}"
	)
